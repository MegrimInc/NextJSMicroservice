...
export default function InventoryPage() {
  ...
  const updateItemCategories = (itemId: number, newCategoryIds: number[]) => {
    setItems((prev) =>
      prev.map((it) =>
        it.itemId === itemId ? { ...it, categoryIds: newCategoryIds } : it
      )
    );
    markEdited(itemId);
  };
  ...
  const updateItemImage =
    (itemId: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setItems((prev) =>
        prev.map((it) =>
          it.itemId === itemId ? { ...it, imageFile: file } : it
        )
      );
      markEdited(itemId);
    };
          ...
          <tr className="hover:bg-gray-100">
            ...
            <Td>
              <Input
                ...
                onChange={(e) =>
                  setNewItem({ ...newItem, gratuityPercent: +e.target.value })
                }
              />
            </Td>
            <Td>
...
const Th = ({
  ...
  <th className={`p-2 text-left uppercase tracking-wide ${className}`}>
    {children}
  </th>
);
