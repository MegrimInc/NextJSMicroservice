...
export default function InventoryPage() {
  ...
  const saveItem = async (it: Item) => {
    try {
      ...
      if (it.imageFile) {
        const webpFile = await convertToWebP(it.imageFile);
      ...
      const body: { [k: string]: any } = { ...it, image: imageUrl };
      fixNums(body);
      ...
      setItems((p) =>
        p.map((x) =>
          x.itemId === it.itemId
            ? { ...x, imageUrl, imageFile: undefined }
            : x
        )
      );
      setDirty((p) => {
  ...
  const addItem = async () => {
    try {
      ...
      if (newItem.imageFile) {
        const webpFile = await convertToWebP(newItem.imageFile);
        const { url, key } = await getPresignedUrl(webpFile);
        await uploadToS3(url, webpFile);
        imageUrl = `https://${S3_BUCKET}.s3.${S3_REGION}.amazonaws.com/${key}`;
      }

      const body: { [k: string]: any } = { ...newItem, image: imageUrl };
      fixNums(body);
      delete body.imageFile;
      delete body.imageUrl;
      ...
      setItems((p) =>
        [...p, {
          ...created,
          imageUrl: created.image ?? created.imageUrl,
