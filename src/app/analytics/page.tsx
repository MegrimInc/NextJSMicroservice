    ...
    return (
        <div className="min-h-screen bg-gray-100 text-black py-8 px-4">
            <div className="max-w-7xl mx-auto">
                <h1 className="text-5xl font-extrabold text-center mb-8">Analytics Dashboard</h1>
                ...
                {generalData && (
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
                        ...
                        <Card title="Units Sold (pts)" value={`${generalData.itemsPoints}`} />
                            ...
                            <>
                                <table className="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tr>
                                        ...
                                        <th className="px-4 py-2 text-left text-sm font-semibold">With $</th>
                                        <th className="px-4 py-2 text-left text-sm font-semibold">With Points</th>
                                    </tr>
                                    </thead>
                                    ...
                                    {drinkCounts.slice(0, visibleCount).map((d) => (
                                        <tr key={d.drinkId} className="hover:bg-gray-100">
                                            ...
                                            <td className="px-4 py-2">${d.doublePrice.toFixed(2)}</td>
                                ...
                                <div className="mt-4 flex justify-center space-x-4">
                                    {!isAllVisible && (
                                        <>
                                            <button onClick={showMore} className="px-4 py-2 bg-green-600 text-white rounded">
                                                Show More
                                            </button>
                                            <button onClick={showAll} className="px-4 py-2 bg-blue-600 text-white rounded">
                                                Show All
                                            </button>
                                        </>
                                    )}
                                    {visibleCount > 10 && (
                                        <button onClick={showLess} className="px-4 py-2 bg-red-400 text-white rounded">
                                            Show Less
                                        </button>
                                    )}
                                </div>
                ...
                <div className="bg-white shadow-lg rounded-lg mb-8">
                    ...
                    <div className="p-6">
                        <div className="flex flex-col md:flex-row md:items-center md:space-x-4 mb-4">
                            ...
                            />
                            <button onClick={searchOrdersByDay} className="mt-2 md:mt-0 px-4 py-2 bg-purple-600 text-white rounded">
                                Search
                            </button>
                        </div>
                        ...
                        {ordersByDay.length ? (
                            <table className="min-w-full divide-y divide-gray-300">
                                ...
                                {ordersByDay.map((o) => (
                                    <tr key={o.orderId} className="hover:bg-gray-100">
                                        ...
                                        <td className="px-4 py-2">${o.totalRegularPrice.toFixed(2)}</td>
                            ...
                            <p className="text-center text-gray-600">No orders for that day.</p>
                        )}
                    </div>
                </div>
            </div>
